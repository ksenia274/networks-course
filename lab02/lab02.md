...
<img src="images/1_POST.png" width=500 />

Получаем данные конкретного продукта (GET)

<img src="images/2_GET.png" width=500 />

Получаем данные всех продуктов (GET)

<img src="images/3_GET_ALL.png" width=500 />

Обновляем данные (PUT) 

<img src="images/4_PUT.png" width=500 />

Удаляем (DELETE)

<img src="images/5_DELETE.png" width=500 />

Получаем иконку

<img src="images/6_GET_ICON.png" width=500 />
...
#### Решение
Первый пакет пройдет путь за $N \dfrac{L}{R}$. 
Тогда второй пакет - за $(N + 1) \dfrac{L}{R}$, потому что пакеты проходят по очереди, по соединению может проходить 1 пакет.

Третий - за $(N + 2) \dfrac{L}{R}$,

P-ый - $(N + P - 1) \dfrac{L}{R}$.
...
#### Решение
Пакеты не будут стоять в очереди после прохождения первого канала, так как у него самая низкая скорость передачи данных. Посчитаем, сколько будет занимать передача всего файла по первому каналу.

$\dfrac{5 \ МБ * 8}{200 \ Кбит/с} = 204,8 ≈ 200$ с.

Когда последний пакет будет проходить первый канал, остальные пакеты дойдут до хоста. Тогда рассчитаем, сколько потребуется времени последнему пакету для прохождения последних 2х каналов. Пусть размеры пакетов приблизительно равны между собой и равны X Кбит.

Тогда получится $\dfrac{Х \ Кбит}{3 \ Мбит/с} + \dfrac{Х \ Кбит}{2 \ Мбит/с} ≈  \dfrac{Х}{1200}$ c.

Сложим и получим ответ: приблизительно $200 + \dfrac{Х}{1200}$ с.
...
#### Решение
$p = 0,2$ - вероятность того, что пользователь использует канал. Заметим, что пропускная способность равна $2 \ Мбит/с = 2048 \ Кбит/с$. Это значит, что только 20 пользователей одновременно могут использовать канал, а вероятность того, что канал используют одновременно от 20 до 60 пользователей равна 0. Найдем вероятность, что одновременно используют канал от 12 до 20 пользователей по формуле Бернулли:

$P_{>=12} =  P_{12 <= users <= 20} = \sum_{i=12}^{20} C_{60}^{i} p^i (1-p)^{60-i} ≈ 0,547$
...
#### Решение
Вспомним формулу из 1го задания: $(N + P - 1) \dfrac{L}{R}$. 
Здесь $N = 3$, $P = \dfrac{X}{S}$, $L = 80 + S$. Подставим и преобразуем. 

$(2 + \dfrac{Х}{S})\dfrac{S + 80}{R} = \dfrac{2S}{R} + \dfrac{160 + X}{R} + \dfrac{80X}{SR}$

Минимизируем, через производную.

$\dfrac{2}{R} - \dfrac{80X}{S^2R} = 0$
...
#### Решение
1. Формула для задержки передачи: $\dfrac{L}{R}$

$\dfrac{I \cdot L}{R (1 – I)} + \dfrac{L}{R} = \dfrac{L}{R (1 – I)}$

Используем формулу интенсивности.

$\dfrac{L}{R (1 – I)} = \dfrac{1}{ \dfrac{R}{L} - a}$

Итоговая формула: $\dfrac{1}{\dfrac{R}{L} - a}$

2. Пусть $q = \dfrac{L}{R}$

Зависимость тогда будет описываться так: $\dfrac{1}{\dfrac{1}{q} - a}$.

Если значение a достаточно маленькое, то зависимость близка к линейной. 
