...
#### Демонстрация работы
<img src="images/1A1.png" width=500 />
<img src="images/1A2.png" width=500 />
Логи приложены
...
3. Используя nslookup, найдите веб-сервер, имеющий несколько IP-адресов. Сколько IP-адресов имеет веб-сервер вашего учебного заведения?
   - <img src="images/2A3.png" width=500 />
   У СПбГУ 1 сервер
   - <img src="images/2A4.png" width=500 />
...
1. Найдите DNS-запрос и ответ на него. С использованием какого транспортного протокола
   ...
   - Они отправлены с использованием протокола UDP.
   <img src="images/2B1.png" width=500 />
2. Какой порт назначения у запроса DNS?
   - 53
   <img src="images/2B2.png" width=500 />
3. На какой IP-адрес отправлен DNS-запрос? Используйте ipconfig для определения IP-адреса
   ...
   - 192.168.0.1
   <img src="images/2B3.png" width=500 />
   - Да, адреса совпадают
   <img src="images/2B4.png" width=500 />
4. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ...
   <img src="images/2B5.png" width=500 />
   - Нет
   <img src="images/2B6.png" width=500 />
5. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - 2 ответа
   <img src="images/2B7.png" width=500 />
   - имя хоста, тип ресурсной записи, класс, TTL, размер данных, ip адрес.
...
1. Каков порт назначения в запросе DNS? Какой порт источника в DNS-ответе?
   - 53
   <img src="images/2C1.png" width=500 />
   - 53
   <img src="images/2C2.png" width=500 />
2. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию?
   - Да, совпадает
   <img src="images/2C3.png" width=500 />
   - <img src="images/2C4.png" width=500 />
3. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?
   - Типа А
   <img src="images/2C5.png" width=500 />
   - Ответов нет
   <img src="images/2C6.png" width=500 />
4. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в каждом?
   - 1 ответ
   <img src="images/2C7.png" width=500 />
   - В нем содержится имя сервера, тип ресурсной записи, класс, TTL, размер данных, ip адрес.
...
3. Проанализируйте ответное сообщение DNS. Имена каких DNS-серверов университета в
   ...
   <img src="images/2D3.png" width=500 />
   - Нет адресов
...
1. На какой IP-адрес отправлен DNS-запрос? Совпадает ли он с адресом локального DNS-сервера, установленного по умолчанию? 
   ...
   - Нет, не совпадает
   <img src="images/2E1.png" width=500 />
2. Проанализируйте сообщение-запрос DNS. Запись какого типа запрашивается? Содержатся
   ли в запросе какие-нибудь «ответы»?
   - Тип А
   <img src="images/2E2.png" width=500 />
   - Ответов нет
3. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - 2 ответа
   <img src="images/2E3.png" width=500 />
   - В первом ответе содержится имя сервера, тип ресурсной записи, класс, TTL, размер данных, и CNAME. Во втором вместо CNAME ip адрес.

### Е. Сервисы whois (2 балла)
1. Что такое база данных whois?
   - Общедоступная база данных, которая содержит информацию о зарегистрированных доменных именах, включая данные о владельцах доменов, их контактных деталях, дате регистрации и сроке действия домена. 
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - Использую whois.icann.org.
   <img src="images/2F1.png" width=500 />
   - <img src="images/2F2.png" width=500 />
3. Используйте команду nslookup на локальном хосте, чтобы послать запросы трем конкретным
   ...
   - <img src="images/2F4.png" width=500 />
   - <img src="images/2F5.png" width=500 />
   - <img src="images/2F6.png" width=500 />
