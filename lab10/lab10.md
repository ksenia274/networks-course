...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Time to live, Header Checksum, Identification
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняется: Source Address, Version, Header Length, Differentiated Services Field, Flags, Fragment Offset, Protocol
      - Какие поля не должны меняться в общем случае: Version, Header Length, Differentiated Services Field, Source Address, Protocol
      - Какие поля должны меняться: Identification, Header Checksum, Destination Address
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Иногда увеличивается на 1, иногда нет закономерности.
      - Нет, все зависит от реализации в узле.
...
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - Identification 0, TTL разный: 250, 249, 244.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Меняются Total Length, Flags, Fragment Offset, Header Checksum.
